...
class Menu<T> {
  ...
  async generateKeyboard(data: T): Promise<InlineKeyboard> {
    ...
    this.items.forEach((row, rowIndex) => {
      row.forEach((item, colIndex) => {
        ...
        const callbackId = MenuGenerator.toCallbackId(
          this.hashedId,
          rowIndex,
          colIndex,
          keyboardId,
  ...
  async call(row: number, col: number, keyboardId: string) {
    ...
    if (!callback)
      throw new Error(`Callback not found for buttonId(row,col): ${buttonId}`);

    const data = await this.dataStorage.read(keyboardId);
    if (!data)
      throw new Error(
        `Data in redis not found for buttonId(row,col): ${buttonId}`,
      );
...
class MenuGenerator<C extends Context> implements MiddlewareObj<C> {
  static toCallbackId(
    ...
    if (row > CONSTANTS.maxRows || col > CONSTANTS.maxCols) {
      logger.warn(
        { row, col },
        "[MenuGen] Asking to create a callbackId with row or col too high!",
      );
      return null;
    }
